...
//    char* envp[4] = { "e1", "e2", "e3", NULL };
    ...
    char* envp[4] = { "PATH=bin:rootfs/bin", "HOME=rootfs/bin", "USER=ROOT",
    NULL };
...
int do_execv(char* bin_name, char ** argv, char** envp) {
    ...
    self_ref_write(PT, tmp_vir_addr,
            allocate_page_user() | PTE_P | PTE_U | PTE_W);
...
//    argc += 1;
    ...
    if (envc > 0) {
        execv_task->mm->env_end = DO_EXECV_TMP_ADDR_TRANSLATE(
                (uint64_t ) envp[envc - 1]);
        while (envc-- > 0) {
            // envp[envc] = (char*) tmp--;
            *((uint64_t*) tmp) = DO_EXECV_TMP_ADDR_TRANSLATE(
                    (uint64_t ) envp[envc]);
            tmp = (uint64_t*) tmp - 1;
        }
        execv_task->mm->env_start = DO_EXECV_TMP_ADDR_TRANSLATE(
                (uint64_t ) envp[0]);
    ...
    if (argc > 0) {
        ...
        execv_task->mm->arg_end = DO_EXECV_TMP_ADDR_TRANSLATE(
                (uint64_t ) argv[argc - 1]);
        ...
        execv_task->mm->arg_start = DO_EXECV_TMP_ADDR_TRANSLATE(
                (uint64_t ) argv[0]);
    }
    ...
    while (vma != NULL) {
        ...
        while (start_addr < end_addr) {
            ...
            if (content) {
                ...
                map_virmem_to_phymem(start_addr, content, USERPT, USER_MAP,
                NOT_NEED_PTE_FLAGS);
...
int do_fork() {
    ...
    for (int i = 1; i <= 3; i++) {
        *((uint64_t *) new_task->init_kern - i) =
                *((uint64_t *) current->init_kern - i);
    }
