...
/* base address and limit fields is ignored */
/* segment base is treated as 0 */
/* readable bit in the type field is ignored */
...
#define L             (0x20000000000000)  /*** long mode--64-bit flag ***/
#define D             (0x40000000000000)  /*** default op size ***/
#define W             (0x00020000000000)  /*** writable data segment ***/

struct sys_segment_descriptor {/* ##128 bit long bit field */
...
void reload_gdt() {
    ...
    //print gdtr(register)
    //for (int i = 0; i <7; i++){
		//printf("%p\n",(int)((gdt[i] & 0xffffffff00000000)>>32));
		//printf("%p\n",(int)(gdt[i] & 0xffffffff));
	//}
}
